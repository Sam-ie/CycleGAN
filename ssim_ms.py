...
def tf_ssim(img1, img2, cs_map=False, mean_metric=True, size=11, sigma=1.5):
    ...
    if cs_map:
        ...
        #        value = ((2*mu1_mu2 + C1)*(2*sigma12 + C2))/((mu1_sq + mu2_sq + C1)*
        #                    (sigma1_sq + sigma2_sq + C2))
        value = tf.div(tf.multiply(tf.add(tf.multiply(2, mu1_mu2), C1), tf.add(tf.multiply(2, sigma12), C2)),
                       tf.multiply(tf.add(tf.add(mu1_sq, mu2_sq), C1), tf.add(tf.add(sigma1_sq, sigma2_sq), C2)))
